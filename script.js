...
/* ==========================================================================
   FEATURE: Contact Form Validation
   Vanilla validation (empty checks + email regex), styled inline errors and
   a fake-send loading state.

   ── HOW TO MAKE THIS FORM ACTUALLY SEND ──────────────────────────────────
   OPTION A — mailto (works right now, no backend):
       Already implemented below; it opens the visitor's mail client.
   OPTION B — Formspree (recommended, free tier):
       1. Create a form at https://formspree.io and copy your endpoint.
       2. Set USE_ENDPOINT = 'https://formspree.io/f/XXXXXXX' below.
   OPTION C — Netlify Forms:
       1. Add   netlify   and   name="contact"   to the <form> in index.html.
       2. Set USE_ENDPOINT = '/' and Netlify captures the POST automatically.
   ========================================================================== */
(function initContactForm() {
  const form = $('#contact-form');
  if (!form) return;

  const USE_ENDPOINT = '';        // EDIT: paste your Formspree/Netlify endpoint here
  const MY_EMAIL = 'gokulamvamshi@example.com';   // EDIT: your real email (mailto fallback)
  ...
  const fields = [
    { input: $('#f-name'),    err: $('#err-name'),    label: 'name',    min: 2,  test: v => v.trim().length >= 2,        msg: 'Please enter your name (2+ characters).' },
    { input: $('#f-email'),   err: $('#err-email'),   label: 'email',            test: v => EMAIL_RE.test(v.trim()),     msg: 'Please enter a valid email address.' },
    { input: $('#f-subject'), err: $('#err-subject'), label: 'subject',          test: v => v.trim().length >= 3,        msg: 'Subject needs at least 3 characters.' },
    { input: $('#f-message'), err: $('#err-message'), label: 'message',          test: v => v.trim().length >= 10,       msg: 'Tell me a bit more — 10 characters minimum.' }
  ];
  ...
  form.addEventListener('submit', async e => {
    ...
    try {
      if (USE_ENDPOINT) {
        // --- Real submission path (Formspree / Netlify / your own API) ---
        ...
        // --- mailto fallback: opens the visitor's mail client ---
        await new Promise(r => setTimeout(r, 900));   // let the spinner breathe
        const body = encodeURIComponent(`${data.message}\n\n— ${data.name} (${data.email})`);
        window.location.href = `mailto:${MY_EMAIL}?subject=${encodeURIComponent(data.subject)}&body=${body}`;
      }

      btn.classList.remove('is-loading');
      status.textContent = '// MESSAGE READY — THANK YOU, I\'LL REPLY SOON';
      status.className = 'form__status mono is-ok';
      form.reset();
      setTimeout(() => { status.textContent = ''; }, 6000);
    } catch (err) {
      btn.classList.remove('is-loading');
      status.textContent = '// TRANSMISSION FAILED — EMAIL ME DIRECTLY INSTEAD';
      status.className = 'form__status mono is-bad';
    }
  });
})();
